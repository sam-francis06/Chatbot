...
function handleInput() {
    ...
    setTimeout(() => {
        ...
        const matches = findRelevantLaws(input);
        ...
        if (matches.length > 0) {
          ...
          `;
            ...
            // Add related laws suggestions
            if (matches.length === 1) {
                const relatedLaws = findRelatedLaws(matches[0], 3);
                if (relatedLaws.length > 0) {
                    const relatedContent = `
              <div class="related-laws">
                <p><strong>📚 Related Laws:</strong></p>
                <div class="related-buttons">
                  ${relatedLaws.map(law =>
                        `<button class="related-law-button" onclick="sendQuickMessage('${law.law}')">${law.section} - ${law.law}</button>`
                    ).join('')}
                </div>
              </div>
            `;
                    addMessage(relatedContent, 'bot-message');
                }
            }
        } else {
          ...
          <div class="suggested-topics">
            <p><strong>Common topics you can ask about:</strong></p>
            <div class="topic-buttons">
              <button class="topic-button" onclick="sendQuickMessage('data privacy')"><i class="fas fa-shield-alt"></i> Data Privacy</button>
              <button class="topic-button" onclick="sendQuickMessage('cyberbullying')"><i class="fas fa-exclamation-triangle"></i> Cyberbullying</button>
              <button class="topic-button" onclick="sendQuickMessage('online fraud')"><i class="fas fa-user-secret"></i> Online Fraud</button>
              <button class="topic-button" onclick="sendQuickMessage('digital rights')"><i class="fas fa-gavel"></i> Digital Rights</button>
            </div>
          </div>
        `;
            addMessage(suggestedTopics, 'bot-message');
        }
...
function findRelatedLaws(currentLaw, count) {
    return laws
        .filter(law => law.section !== currentLaw.section &&
            law.keywords.some(k1 => currentLaw.keywords.some(k2 => k1 === k2)))
        .slice(0, count);
}
